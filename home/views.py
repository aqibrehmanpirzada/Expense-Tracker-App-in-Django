...
def expense_week(request):
    ...
    expense_per_category = Addmoney_info.objects.filter(add_money='Expense', Date__range=(start_date, end_date)).values('Category__name').annotate(total_expense=Sum('quantity'))
...
def income_week(request):
    start_date = timezone.now().date() - datetime.timedelta(days=7)
    end_date = timezone.now().date() - datetime.timedelta(days=1)
    expense_per_category = Addmoney_info.objects.filter(add_money='Income', Date__range=(start_date, end_date)).values('Category__name').annotate(total_expense=Sum('quantity'))
    # The above query filters the records to include only expenses and groups them by Category,
    # then calculates the sum of 'quantity' for each group
    data = {
        'labels': [item['Category__name'] for item in expense_per_category],
